...
function search() {
    ...
    $("#alt").append("<thead><tr><th><a id='id' onclick='sort(this.id)'>Id</a></th><th><a id='nome' onclick='sort(this.id)'>Nome</a></th><th><a id='descrizione' onclick='sort(this.id)'>Descrizione</a></th><th><a id='durataprevista' onclick='sort(this.id)'>Durata Prevista</a></th><th><a id='tipo' onclick='sort(this.id)'>Tipo</a></th><th>    </th></tr></thead>");
    //controllo dati inseriti e ricerca
    if(id == "" && nome == "" && descrizione == "" && durataprevista == "" && tipo == "") {
        for(var i=0; i<proc.length; i++) {
            $("#alt").append("<tbody><tr><td>"+
			proc[i].id+"</td><td>"+
			proc[i].nome+"</td><td>"+
			proc[i].descrizione+"</td><td>"+
			proc[i].durataprevista+"</td><td>"+
			proc[i].tipo+"</td><td>"+
			"<a href='#' class='button special fit'>Esegui</a></tr></tbody>");
        }
        print = true;
    } else {
